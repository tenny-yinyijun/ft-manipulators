...
def main_2d(dof, res, num_examples, datasetname):
    all_reachability_maps = np.zeros((num_examples, res, res)) # TODO fixed resolution
...
def main_3d(dof, res, num_examples, datasetname):
    dh_params = np.zeros((4, 4))
    dh_params[0,1] = 0.089
    dh_params[1:,1] = 0.10475

    all_reachability_maps = np.zeros((num_examples, res, res, res))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--res", type=int, required=True, help="Resolution of the reachability space")
    ...
    dataset_name = f"data/{args.dim}d_{args.dof}dof_res{args.res}_{args.num_examples}_{timestamp}"

    if not os.path.exists(dataset_name):
        os.makedirs(dataset_name)

    if args.dim == 2:
        main_2d(args.dof, args.res, args.num_examples, dataset_name)
    else:
        main_3d(args.dof, args.res, args.num_examples, dataset_name)